...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("uim_moneda_referencia_name");
			if (d != null)
			{
			name  = "b_referenced_coin";
			label = "${lbl:b_referenced_coin}";
		...
		d = rs.getString("uim_moneda_referencia");
			if (d != null)
			{
			name  = "b_referenced_coin";
			label = "${lbl:b_referenced_coin}";
		...
		d = rs.getString("uim_descripcion");
			if (d != null)
			{
			name  = "b_description";
