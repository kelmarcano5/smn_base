...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tim_valor_ini");
			if (d != null)
			{
			name  = "b_value_from";
			label = "${lbl:b_value_from}";
		...
		d = rs.getString("tim_fecha_vigencia_fin");
			if (d != null)
			{
			name  = "b_validation_date_to";
			label = "${lbl:b_validation_date_to}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tim_fecha_vigencia_ini");
			if (d != null)
			{
			name  = "b_validation_date_fromb_validation_date";
			label = "${lbl:b_validation_date_fromb_validation_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_unidades_impositivas_id_name");
			if (d != null)
			{
			name  = "b_tax_unit";
			label = "${lbl:b_tax_unit}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tim_valor_fin");
			if (d != null)
			{
			name  = "b_value_to";
			label = "${lbl:b_value_to}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_unidades_impositivas_id");
			if (d != null)
			{
			name  = "b_tax_unit";
			label = "${lbl:b_tax_unit}";
