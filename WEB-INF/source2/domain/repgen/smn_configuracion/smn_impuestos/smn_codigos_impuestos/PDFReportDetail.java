...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("imp_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("imp_tipo_persona");
			if (d != null)
			{
			name  = "b_kind_of_person";
			label = "${lbl:b_kind_of_person}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("imp_unidad_impositiva_name");
			if (d != null)
			{
			name  = "b_tax_unit";
		...
		d = rs.getString("imp_unidad_impositiva");
			if (d != null)
			{
			name  = "b_tax_unit";
			label = "${lbl:b_tax_unit}";
		...
		d = rs.getString("imp_tipo_contribuyente");
			if (d != null)
			{
			name  = "b_type_of_taxpayer";
			label = "${lbl:b_type_of_taxpayer}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
