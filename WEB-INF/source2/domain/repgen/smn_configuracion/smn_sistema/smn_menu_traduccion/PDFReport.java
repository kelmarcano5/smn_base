...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("mnt_title");
			if (d != null)
			{
			name  = "b_menu_title";
			label = "${lbl:b_menu_title}";
		...
		d = rs.getString("mnt_menu_id_name");
			if (d != null)
			{
			name  = "b_menu";
			label = "${lbl:b_menu}";
		...
		d = rs.getString("mnt_app_id_name");
			if (d != null)
			{
			name  = "b_application";
		...
		d = rs.getString("mnt_idioma");
			if (d != null)
			{
			name  = "s_profile_lang";
			label = "${lbl:s_profile_lang}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
