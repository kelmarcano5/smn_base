package domain.repgen.smn_configuracion.smn_entidades.smn_rel_entidad_estructura;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_entidades_id_name");
			if (d != null)
			{
			name  = "b_entities";
		...
		d = rs.getString("rec_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_entidades_id");
			if (d != null)
			{
			name  = "b_entities";
			label = "${lbl:b_entities}";
		...
		d = rs.getString("rec_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
