...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("crp_unidad_negocio");
			if (d != null)
			{
		...
		d = rs.getString("crp_unidad_negocio_name");
			if (d != null)
			{
			name  = "b_business_unit";
			label = "${lbl:b_business_unit}";
		...
		d = rs.getString("crp_moneda_name");
			if (d != null)
			{
			name  = "b_coin";
			label = "${lbl:b_coin}";
