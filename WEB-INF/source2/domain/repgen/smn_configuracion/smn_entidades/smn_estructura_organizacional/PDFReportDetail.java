...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("eor_clase_auxiliar_name");
			if (d != null)
			{
		...
		d = rs.getString("smn_configuracion_estructura_id_name");
			if (d != null)
			{
			name  = "b_structure_configuration";
			label = "${lbl:b_structure_configuration}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("eor_centro_costo_name");
			if (d != null)
			{
			name  = "b_cost_centers";
		...
		d = rs.getString("eor_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary";
		...
		d = rs.getString("eor_centro_costo");
			if (d != null)
			{
			name  = "b_cost_centers";
			label = "${lbl:b_cost_centers}";
		...
		d = rs.getString("smn_configuracion_estructura_id");
			if (d != null)
			{
			name  = "b_structure_configuration";
			label = "${lbl:b_structure_configuration}";
		...
		d = rs.getString("eor_clase_auxiliar");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
