...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("suc_unidad_negocio");
			if (d != null)
			{
		...
		d = rs.getString("suc_unidad_negocio_name");
			if (d != null)
			{
			name  = "b_business_unit";
			label = "${lbl:b_business_unit}";
		...
		d = rs.getString("suc_empresa_name");
			if (d != null)
			{
			name  = "b_entity";
			label = "${lbl:b_entity}";
