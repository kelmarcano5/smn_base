package domain.repgen.smn_configuracion.smn_codificaciones.smn_rel_nivel_estruc_atributo;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_nivel_estructura_id");
			if (d != null)
			{
			name  = "b_nivel_estructura";
			label = "${lbl:b_nivel_estructura}";
		...
		d = rs.getString("smn_atributo_item_id");
			if (d != null)
			{
			name  = "b_atributos_item";
			label = "${lbl:b_atributos_item}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_atributo_item_id_name");
			if (d != null)
			{
			name  = "b_atributos_item";
			label = "${lbl:b_atributos_item}";
		...
		d = rs.getString("smn_nivel_estructura_id_name");
			if (d != null)
			{
			name  = "b_nivel_estructura";
			label = "${lbl:b_nivel_estructura}";
