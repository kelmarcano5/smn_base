package domain.repgen.smn_configuracion.smn_variable;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("var_codigo");
			if (d != null)
			{
			name  = "b_var_codigo";
			label = "${lbl:b_var_codigo}";
		...
		d = rs.getString("var_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("var_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
		...
		d = rs.getString("var_descripción");
			if (d != null)
			{
			name  = "b_var_descripcion";
			label = "${lbl:b_var_descripcion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
