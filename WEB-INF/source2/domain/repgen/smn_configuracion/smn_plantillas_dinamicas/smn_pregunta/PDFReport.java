package domain.repgen.smn_configuracion.smn_pregunta;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("prg_codigo");
			if (d != null)
			{
			name  = "b_prg_codigo";
			label = "${lbl:b_prg_codigo}";
		...
		d = rs.getString("cue_idioma");
			if (d != null)
			{
			name  = "s_profile_lang}@{\${def:locale}";
			label = "${lbl:s_profile_lang}@{\${def:locale}}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("prg_descripcion");
			if (d != null)
			{
			name  = "b_prg_descripcion";
			label = "${lbl:b_prg_descripcion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cue_fecha_registro_fin");
			if (d != null)
			{
			name  = "s_loginhist_col2}@{\${def:time}";
			label = "${lbl:s_loginhist_col2}@{\${def:time}}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("prg_tipo_respuesta");
			if (d != null)
			{
			name  = "b_prg_tipo_respuesta";
			label = "${lbl:b_prg_tipo_respuesta}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cue_fecha_registro_ini");
			if (d != null)
			{
			name  = "s_loginhist_col2}@{\${def:time}";
			label = "${lbl:s_loginhist_col2}@{\${def:time}}";
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_smn_tipo_variable_id";
			label = "${lbl:b_smn_tipo_variable_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
