package domain.repgen.smn_procesos.smn_rel_proceso_respuesta;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_smn_cuestionario_id";
			label = "${lbl:b_smn_cuestionario_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_respuesta_id");
			if (d != null)
			{
			name  = "b_smn_respuesta_id";
			label = "${lbl:b_smn_respuesta_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
