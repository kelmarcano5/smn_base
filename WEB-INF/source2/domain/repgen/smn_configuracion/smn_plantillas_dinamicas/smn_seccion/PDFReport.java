package domain.repgen.smn_configuracion.smn_seccion;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("sec_codigo");
			if (d != null)
			{
			name  = "b_sec_codigo";
			label = "${lbl:b_sec_codigo}";
		...
		d = rs.getString("sec_descripcion");
			if (d != null)
			{
			name  = "b_sec_descripcion";
			label = "${lbl:b_sec_descripcion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
