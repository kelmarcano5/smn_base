package domain.repgen.smn_configuracion.smn_cuestionario;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
			...
			if (d != null)
			{
			name  = "b_smn_modulo_rf";
			label = "${lbl:b_smn_modulo_rf}";
		...
		d = rs.getString("smn_campo_rf_name");
			if (d != null)
			{
			name  = "b_smn_campo_rf";
			label = "${lbl:b_smn_campo_rf}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_esquema_rf_name");
			if (d != null)
			{
			name  = "b_smn_esquema_rf";
			label = "${lbl:b_smn_esquema_rf}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cue_codigo");
			if (d != null)
			{
			name  = "b_cue_codigo";
			label = "${lbl:b_cue_codigo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cue_descripcion");
			if (d != null)
			{
			name  = "b_cue_descripcion";
			label = "${lbl:b_cue_descripcion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
