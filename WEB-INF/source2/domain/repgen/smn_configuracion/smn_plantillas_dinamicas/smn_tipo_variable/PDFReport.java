package domain.repgen.smn_configuracion.smn_tipo_variable;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tva_descripcion");
			if (d != null)
			{
			name  = "b_tva_descripcion";
			label = "${lbl:b_tva_descripcion}";
		...
		d = rs.getString("tva_codigo");
			if (d != null)
			{
			name  = "b_tva_codigo";
			label = "${lbl:b_tva_codigo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tva_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("tva_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
