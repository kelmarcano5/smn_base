...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("bar_vigencia_desde");
			if (d != null)
			{
			name  = "b_validation_date_from";
			label = "${lbl:b_validation_date_from}";
		...
		d = rs.getString("bar_entidad_name");
			if (d != null)
			{
			name  = "b_entity";
			label = "${lbl:b_entity}";
		...
		d = rs.getString("bar_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("bar_vigencia_hasta");
			if (d != null)
			{
