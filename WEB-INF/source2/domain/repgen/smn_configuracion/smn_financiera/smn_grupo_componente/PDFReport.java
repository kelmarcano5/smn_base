package domain.repgen.smn_configuracion.smn_financiera.smn_grupo_componente;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("gco_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("gco_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("gco_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
