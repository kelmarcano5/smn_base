package domain.repgen.smn_configuracion.smn_financiera.smn_vencimiento_cabecera;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("tve_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("tve_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("tve_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
