...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("cba_moneda_name");
			if (d != null)
			{
			name  = "b_coin";
			label = "${lbl:b_coin}";
		...
		d = rs.getString("cba_entidades_financieras_name");
			if (d != null)
			{
			name  = "b_financial_entity";
			label = "${lbl:b_financial_entity}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cba_entidades_financieras");
			if (d != null)
			{
			name  = "b_financial_entity";
			label = "${lbl:b_financial_entity}";
