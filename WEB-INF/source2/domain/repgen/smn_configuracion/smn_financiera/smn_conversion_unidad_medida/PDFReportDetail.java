package domain.repgen.smn_configuracion.smn_financiera.smn_conversion_unidad_medida;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cou_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("smn_unidad_medida_origen_rf");
			if (d != null)
			{
			name  = "b_unidad_medida_origen";
			label = "${lbl:b_unidad_medida_origen}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cou_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_unidad_medida_destino_rf");
			if (d != null)
			{
			name  = "b_unidad_medida_destino";
			label = "${lbl:b_unidad_medida_destino}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cou_factor_conversion");
			if (d != null)
			{
			name  = "b_factor_conversion";
			label = "${lbl:b_factor_conversion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_unidad_medida_destino_rf_name");
			if (d != null)
			{
			name  = "b_unidad_medida_destino";
			label = "${lbl:b_unidad_medida_destino}";
		...
		d = rs.getString("smn_unidad_medida_origen_rf_name");
			if (d != null)
			{
			name  = "b_unidad_medida_origen";
			label = "${lbl:b_unidad_medida_origen}";
