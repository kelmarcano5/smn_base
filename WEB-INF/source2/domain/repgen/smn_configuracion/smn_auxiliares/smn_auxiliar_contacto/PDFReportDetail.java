...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("auc_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_tipo_contactos_rf");
			if (d != null)
			{
			name  = "b_type_contact";
			label = "${lbl:b_type_contact}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("auc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("auc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_tipo_contactos_rf_name");
			if (d != null)
			{
			name  = "b_type_contact";
			label = "${lbl:b_type_contact}";
