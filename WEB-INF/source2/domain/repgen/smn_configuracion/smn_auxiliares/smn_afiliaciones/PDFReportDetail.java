...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("afc_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("afc_modulo_name");
			if (d != null)
			{
			name  = "b_module";
			label = "${lbl:b_module}";
		...
		d = rs.getString("afc_afiliado_name");
			if (d != null)
			{
			name  = "b_joined";
			label = "${lbl:b_joined}";
