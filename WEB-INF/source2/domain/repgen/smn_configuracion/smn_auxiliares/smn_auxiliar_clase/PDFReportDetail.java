package domain.repgen.smn_configuracion.smn_auxiliares.smn_auxiliar_clase;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("acl_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_clase_auxiliar_id");
			if (d != null)
			{
		...
		d = rs.getString("acl_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_clase_auxiliar_id_name");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
