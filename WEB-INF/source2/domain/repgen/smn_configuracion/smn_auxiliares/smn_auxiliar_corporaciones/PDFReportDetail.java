...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tcp_moneda_name");
			if (d != null)
			{
			name  = "b_coin";
			label = "${lbl:b_coin}";
		...
		d = rs.getString("tcp_auxiliar_unidad_negocio_name");
			if (d != null)
			{
		...
		d = rs.getString("tcp_auxiliar_unidad_negocio");
			if (d != null)
			{
			name  = "b_outsourced_unit_business";
			label = "${lbl:b_outsourced_unit_business}";
