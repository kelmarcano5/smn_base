package domain.repgen.smn_configuracion.smn_auxiliares.smn_rel_aux_contacto;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_auxiliar_contacto_id_name");
			if (d != null)
			{
			name  = "b_auxiliary_contact";
			label = "${lbl:b_auxiliary_contact}";
		...
		d = rs.getString("smn_auxiliar_contacto_id");
			if (d != null)
			{
			name  = "b_auxiliary_contact";
			label = "${lbl:b_auxiliary_contact}";
