select
	(select
	 ...
	 where 
	 	smn_base.smn_v_auxiliar.smn_v_auxiliar_id = ${fld:eor_auxiliar_filter}) as auxiliar_filter,
	(select
		smn_base.smn_clase_auxiliar.cla_nombre
	 from 
	 	smn_base.smn_clase_auxiliar
	 where 
	 	smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id = ${fld:eor_clase_auxiliar_filter}) as clase_auxiliar_filter,
	(select
		smn_base.smn_centro_costo.cco_descripcion_corta
	 from 
	 	smn_base.smn_centro_costo
	 where 
	 	smn_base.smn_centro_costo.smn_centro_costo_id = ${fld:eor_centro_costo_filter}) as centro_costo_filter,
	(select
		smn_base.smn_configuracion_estructura.coe_codigo
	 from 
	 	smn_base.smn_configuracion_estructura
	 where 
	 	smn_base.smn_configuracion_estructura.smn_configuracion_estructura_id = ${fld:smn_configuracion_estructura_id_filter}) as estructura_organizacional_filter,
	smn_base.smn_v_auxiliar.smn_item,
	smn_base.smn_centro_costo.cco_descripcion_corta,
	smn_base.smn_clase_auxiliar.cla_nombre,
