package domain.repgen.smn_configuracion.smn_usuarios.smn_perfil;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("prf_tipo_acceso");
			if (d != null)
			{
			name  = "b_type_access";
			label = "${lbl:b_type_access}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("prf_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("service_id_name");
			if (d != null)
			{
			name  = "b_service";
			label = "${lbl:b_service}";
		...
		d = rs.getString("prf_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
