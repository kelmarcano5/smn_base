...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rgu_tipo_registro_name");
			if (d != null)
			{
			name  = "b_register_type";
			label = "${lbl:b_register_type}";
		...
		d = rs.getString("rgu_nombres");
			if (d != null)
			{
			name  = "s_profile_fname";
			label = "${lbl:s_profile_fname}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rgu_nro_id_fiscal");
			if (d != null)
			{
			name  = "b_tax_identification_number";
			label = "${lbl:b_tax_identification_number}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rgu_num_doc_oficial");
			if (d != null)
			{
			name  = "b_official_document_number";
			label = "${lbl:b_official_document_number}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rgu_apellidos");
			if (d != null)
			{
			name  = "s_profile_lname";
			label = "${lbl:s_profile_lname}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rgu_tipo_doc_oficial_name");
			if (d != null)
			{
			name  = "b_official_document_type";
			label = "${lbl:b_official_document_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
