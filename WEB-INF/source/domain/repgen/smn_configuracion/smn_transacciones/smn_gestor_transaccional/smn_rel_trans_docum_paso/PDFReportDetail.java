package domain.repgen.smn_configuracion.smn_transacciones.smn_gestor_transaccional.smn_rel_trans_docum_paso;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_paso_id_name");
			if (d != null)
			{
			name  = "b_paso";
			label = "${lbl:b_paso}";
		...
		d = rs.getString("smn_documentos_generales_rf_name");
			if (d != null)
			{
			name  = "b_documentos_generales";
			label = "${lbl:b_documentos_generales}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_transaccion_general_rf_name");
			if (d != null)
			{
			name  = "b_transaccion_general";
			label = "${lbl:b_transaccion_general}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tpa_fecha_registro");
			if (d != null)
			{
			name  = "b_store_date";
