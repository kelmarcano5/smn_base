package domain.repgen.smn_configuracion.smn_transacciones.smn_gestor_transaccional.smn_nivel_aprobacion;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("nap_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_configuracion_estructura_id_name");
			if (d != null)
			{
			name  = "b_configuracion_estructura";
			label = "${lbl:b_configuracion_estructura}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("nap_fecha_registro");
			if (d != null)
			{
			name  = "b_store_date";
