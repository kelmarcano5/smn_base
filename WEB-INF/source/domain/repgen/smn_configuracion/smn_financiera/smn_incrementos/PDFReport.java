package domain.repgen.smn_configuracion.smn_financiera.smn_incrementos;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("inc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("inc_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("inc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("inc_apli_cant_precio");
			if (d != null)
			{
			name  = "b_apply_quantity_price";
			label = "${lbl:b_apply_quantity_price}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
