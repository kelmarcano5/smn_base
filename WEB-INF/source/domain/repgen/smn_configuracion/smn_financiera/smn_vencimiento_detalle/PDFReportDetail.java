package domain.repgen.smn_configuracion.smn_financiera.smn_vencimiento_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tvd_tipo_aplicacion");
			if (d != null)
			{
			name  = "b_type_aplication";
			label = "${lbl:b_type_aplication}";
		...
		d = rs.getString("tvd_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("tvd_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_expiration_header}@{select smn_base.smn_vencimiento_cabecera.smn_vencimiento_cabecera_id as id, smn_base.smn_vencimiento_cabecera.tve_codigo ||'-'|| smn_base.smn_vencimiento_cabecera.tve_descripcion as item FROM smn_base.smn_vencimiento_cabecera";
			label = "${lbl:b_expiration_header}@{select smn_base.smn_vencimiento_cabecera.smn_vencimiento_cabecera_id as id, smn_base.smn_vencimiento_cabecera.tve_codigo ||'-'|| smn_base.smn_vencimiento_cabecera.tve_descripcion as item FROM smn_base.smn_vencimiento_cabecera}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
