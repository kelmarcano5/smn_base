...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("svc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_tipo_servicio_rf_name");
			if (d != null)
			{
			name  = "b_service_type";
			label = "${lbl:b_service_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("svc_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("svc_inactivo");
			if (d != null)
			{
			name  = "b_inactive";
			label = "${lbl:b_inactive}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_servicio_rf");
			if (d != null)
			{
			name  = "b_service_type";
			label = "${lbl:b_service_type}";
		...
		d = rs.getString("svc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("svc_frecuencia");
			if (d != null)
			{
			name  = "b_frecuency";
			label = "${lbl:b_frecuency}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
