package domain.repgen.smn_configuracion.smn_financiera.smn_contrato_base;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("ctr_fecha_vencimiento");
			if (d != null)
			{
			name  = "b_date_expired";
			label = "${lbl:b_date_expired}";
		...
		d = rs.getString("ctr_corporacion_rf_name");
			if (d != null)
			{
			name  = "b_corporation";
		...
		d = rs.getString("ctr_corporacion_rf");
			if (d != null)
			{
			name  = "b_corporation";
			label = "${lbl:b_corporation}";
		...
		d = rs.getString("ctr_transaccion_rf");
			if (d != null)
			{
			name  = "b_transaction";
			label = "${lbl:b_transaction}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ctr_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("ctr_transaccion_rf_name");
			if (d != null)
			{
			name  = "b_transaction";
			label = "${lbl:b_transaction}";
		...
		d = rs.getString("ctr_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
