package domain.repgen.smn_configuracion.smn_financiera.smn_baremos_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("bad_tipo_componente");
			if (d != null)
			{
			name  = "b_type";
			label = "${lbl:b_type}";
		...
		d = rs.getString("smn_baremos_id_name");
			if (d != null)
			{
			name  = "b_baremo";
			label = "${lbl:b_baremo}";
		...
		d = rs.getString("bad_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_servicios_rf_name");
			if (d != null)
			{
			name  = "b_service";
			label = "${lbl:b_service}";
		...
		d = rs.getString("smn_servicios_rf");
			if (d != null)
			{
			name  = "b_service";
			label = "${lbl:b_service}";
		...
		d = rs.getString("bad_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
