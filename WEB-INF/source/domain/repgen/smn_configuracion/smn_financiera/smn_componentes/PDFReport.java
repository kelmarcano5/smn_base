...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cmp_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("cmp_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("cmp_forma_calculo");
			if (d != null)
			{
			name  = "b_calculus_form";
			label = "${lbl:b_calculus_form}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cmp_clase_auxiliar_rf_name");
			if (d != null)
			{
		...
		d = rs.getString("smn_grupo_componente_rf");
			if (d != null)
			{
			name  = "b_group_component";
			label = "${lbl:b_group_component}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cmp_clase_auxiliar_rf");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
		...
		d = rs.getString("cmp_tipo_componente");
			if (d != null)
			{
			name  = "b_type_component";
			label = "${lbl:b_type_component}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cmp_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_grupo_componente_rf_name");
			if (d != null)
			{
			name  = "b_group_component";
			label = "${lbl:b_group_component}";
