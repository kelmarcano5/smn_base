package domain.repgen.smn_configuracion.smn_financiera.smn_grupo_titulo_impresion;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("gti_fecha_vigencia_fin");
			if (d != null)
			{
			name  = "b_validation_date";
			label = "${lbl:b_validation_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("gti_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("gti_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("gti_fecha_vigencia_ini");
			if (d != null)
			{
			name  = "b_validation_date";
			label = "${lbl:b_validation_date}";
		...
		d = rs.getString("gti_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
