package domain.repgen.smn_configuracion.smn_financiera.smn_unidad_medida;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("unm_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_tipo_unidad_medida_rf");
			if (d != null)
			{
			name  = "b_measurement_unit_type";
			label = "${lbl:b_measurement_unit_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_unidad_medida_rf_name");
			if (d != null)
			{
			name  = "b_measurement_unit_type";
			label = "${lbl:b_measurement_unit_type}";
		...
		d = rs.getString("unm_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("unm_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
