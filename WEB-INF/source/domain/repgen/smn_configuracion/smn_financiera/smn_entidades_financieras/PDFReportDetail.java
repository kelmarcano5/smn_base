package domain.repgen.smn_configuracion.smn_financiera.smn_entidades_financieras;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("efi_banco_intermediario_name");
			if (d != null)
			{
			name  = "b_intermediary_institution";
			label = "${lbl:b_intermediary_institution}";
		...
		d = rs.getString("efi_banco_intermediario");
			if (d != null)
			{
			name  = "b_intermediary_institution";
			label = "${lbl:b_intermediary_institution}";
		...
		d = rs.getString("efi_contacto");
			if (d != null)
			{
			name  = "b_account_executive";
			label = "${lbl:b_account_executive}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
