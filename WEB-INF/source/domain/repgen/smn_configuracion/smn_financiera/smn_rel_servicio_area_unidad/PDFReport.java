package domain.repgen.smn_configuracion.smn_financiera.smn_rel_servicio_area_unidad;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("rsa_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_service}@{SELECT smn_base.smn_servicios.smn_servicios_id as id, smn_base.smn_servicios.svc_codigo ||'-'|| smn_base.smn_servicios.svc_descripcion AS ITEM FROM smn_base.smn_servicios";
			label = "${lbl:b_service}@{SELECT smn_base.smn_servicios.smn_servicios_id as id, smn_base.smn_servicios.svc_codigo ||'-'|| smn_base.smn_servicios.svc_descripcion AS ITEM FROM smn_base.smn_servicios}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rsa_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
