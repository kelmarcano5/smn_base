package domain.repgen.smn_configuracion.smn_financiera.smn_baremos_detalle_componente;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_componente_rf_name");
			if (d != null)
			{
			name  = "b_components";
			label = "${lbl:b_components}";
		...
		d = rs.getString("bdc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_componente_rf");
			if (d != null)
			{
			name  = "b_components";
			label = "${lbl:b_components}";
		...
		d = rs.getString("smn_baremos_detalle_id_name");
			if (d != null)
			{
			name  = "b_baremos_detail";
			label = "${lbl:b_baremos_detail}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_compuesto_rf");
			if (d != null)
			{
			name  = "b_type";
			label = "${lbl:b_type}";
		...
		d = rs.getString("smn_baremos_detalle_id");
			if (d != null)
			{
			name  = "b_baremos_detail";
			label = "${lbl:b_baremos_detail}";
		...
		d = rs.getString("bdc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
