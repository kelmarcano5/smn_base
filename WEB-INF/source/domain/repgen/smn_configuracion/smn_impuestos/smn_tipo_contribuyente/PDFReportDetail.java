package domain.repgen.smn_configuracion.smn_impuestos.smn_tipo_contribuyente;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("tco_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("tco_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("tco_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
