package domain.repgen.smn_configuracion.smn_auxiliares.smn_rel_modulo_clase_aux;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rma_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_modulos_rf_name");
			if (d != null)
			{
			name  = "b_module";
			label = "${lbl:b_module}";
		...
		d = rs.getString("rma_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
