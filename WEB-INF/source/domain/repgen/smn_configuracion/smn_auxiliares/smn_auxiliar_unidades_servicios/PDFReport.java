...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_auxiliar_areas_servicios_id_name");
			if (d != null)
			{
			name  = "b_outsourced_service_area";
			label = "${lbl:b_outsourced_service_area}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_auxiliar_areas_servicios_id");
			if (d != null)
			{
			name  = "b_outsourced_service_area";
			label = "${lbl:b_outsourced_service_area}";
		...
		d = rs.getString("tns_descripcion");
			if (d != null)
			{
			name  = "b_description";
