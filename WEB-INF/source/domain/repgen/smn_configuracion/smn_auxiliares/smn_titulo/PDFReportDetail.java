package domain.repgen.smn_configuracion.smn_auxiliares.smn_titulo;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tlo_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("tlo_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("tlo_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
