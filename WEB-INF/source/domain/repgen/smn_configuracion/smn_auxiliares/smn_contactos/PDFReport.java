package domain.repgen.smn_configuracion.smn_auxiliares.smn_contactos;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cnt_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("cnt_fecha_registro");
			if (d != null)
			{
			name  = "${lbl:b_store_date}";
			label = "${lbl:b_store_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_auxiliar_id_name");
			if (d != null)
			{
			name  = "b_auxiliary";
		...
		d = rs.getString("smn_auxiliar_id");
			if (d != null)
			{
			name  = "b_auxiliary";
			label = "${lbl:b_auxiliary}";
		...
		d = rs.getString("cnt_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
