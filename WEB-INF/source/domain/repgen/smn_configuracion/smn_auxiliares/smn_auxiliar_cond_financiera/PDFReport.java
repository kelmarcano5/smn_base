...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cfn_clase_auxiliar");
			if (d != null)
			{
		...
		d = rs.getString("cfn_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("cfn_auxiliar_categoria");
			if (d != null)
			{
			name  = "b_auxiliary_category";
			label = "${lbl:b_auxiliary_category}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cfn_auxiliar_categoria_name");
			if (d != null)
			{
			name  = "b_auxiliary_category";
			label = "${lbl:b_auxiliary_category}";
		...
		d = rs.getString("cfn_clase_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary_class";
			label = "${lbl:b_auxiliary_class}";
