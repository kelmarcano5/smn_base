package domain.repgen.smn_configuracion.smn_servicios.smn_rel_unid_grup_prest_contado;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_prestador_servicio_rf");
			if (d != null)
			{
			name  = "b_services_providers}@{SELECT smn_base.smn_prestadores_servicios.smn_prestadores_servicios_id AS ID, smn_base.smn_prestadores_servicios.prs_codigo||'-'|| smn_base.smn_prestadores_servicios.prs_nombres AS ITEM FROM smn_base.smn_prestadores_servicios";
			label = "${lbl:b_services_providers}@{SELECT smn_base.smn_prestadores_servicios.smn_prestadores_servicios_id AS ID, smn_base.smn_prestadores_servicios.prs_codigo||'-'|| smn_base.smn_prestadores_servicios.prs_nombres AS ITEM FROM smn_base.smn_prestadores_servicios}";
		...
		d = rs.getString("smn_unidades_servicios_rf");
			if (d != null)
			{
			name  = "b_service_unit";
		...
		d = rs.getString("smn_grupo_prestador_rf");
			if (d != null)
			{
			name  = "b_groups_of_providers}@{SELECT smn_base.smn_grupos_prestadores.smn_grupos_prestadores_id AS ID, smn_base.smn_grupos_prestadores.gpp_codigo ||'-'|| smn_base.smn_grupos_prestadores.gpp_descripcion AS itemFROM smn_base.smn_grupos_prestadores";
			label = "${lbl:b_groups_of_providers}@{SELECT smn_base.smn_grupos_prestadores.smn_grupos_prestadores_id AS ID, smn_base.smn_grupos_prestadores.gpp_codigo ||'-'|| smn_base.smn_grupos_prestadores.gpp_descripcion AS itemFROM smn_base.smn_grupos_prestadores}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_unidades_servicios_rf_name");
			if (d != null)
			{
			name  = "b_service_unit";
			label = "${lbl:b_service_unit}";
