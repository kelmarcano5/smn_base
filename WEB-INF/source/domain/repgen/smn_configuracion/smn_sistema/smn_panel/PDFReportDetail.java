...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("pan_service_description");
			if (d != null)
			{
			name  = "b_service_description";
			label = "${lbl:b_service_description}";
		...
		d = rs.getString("pan_role_id_name");
			if (d != null)
			{
			name  = "b_role";
			label = "${lbl:b_role}";
		...
		d = rs.getString("pan_app_id_name");
			if (d != null)
			{
			name  = "b_application";
			label = "${lbl:b_application}";
